  ...
  const handleSubmit = async (e) => {
    ...
    if (edit && imageSelected === "") {
      ...
      const res = await axios.post(
        "https://api.cloudinary.com/v1_1/dekkpovmw/image/upload",
        formData
      );
    ...
    edit
      ? await axios.post(`${backendUrl}/editproject`, projectJSON)
    ...
    >
      <Form onSubmit={handleSubmit}>
        <Modal.Header closeButton>
          <Modal.Title>
            {edit ? `Edit ${titlePlaceholder}` : "Add A New Project"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
